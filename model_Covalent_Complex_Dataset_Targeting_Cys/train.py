...
Y=pd.read_csv('data/data1_shuffle.csv',sep=',')['labels']
Y=np.array(Y)
Y = torch.LongTensor(Y)

X=pd.read_csv('data/100dim-complex.csv',sep=',', header=0)
X=np.array(X)
X = torch.Tensor(X)

my_dataset = data.TensorDataset(X, Y)
train_size = len(my_dataset)*2/3
...
def eval(epoch, if_test):
    ...
    if if_test:
        ...
        if acc >=max(test_scores):
            ...
            plt.cla()
            plt.close()
            metrics ={'Epoch': ('%d' % (epoch+1)),'Accuracy': ('%.2f%%' % (acc*100)),
...
def picture(x,y,save_path,z,num):
    plt.cla()
    plt.subplot(1,1,1)
    plt.plot(x,y)
    plt.title("%s accuracy" %(z))
    plt.ylabel("%s accuracy" %(z))
    plt.axhline(y=num, ls="--", c="black",label = "acc = "+str(num))
    plt.legend()
...
def main():
    ...
    picture(range(0, epoch+1), train_scores, save_path, 'train', 0.99)
    picture(range(0, epoch+1), test_scores, save_path, 'test', 0.94)
